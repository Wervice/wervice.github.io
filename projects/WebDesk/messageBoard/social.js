...
function sendCryptEmail() {
    if (confirm("Do you want to send a mail to " + document.getElementById('email').value + "?")) {
        ...
        window.open("mailto:" + document.getElementById('email').value + "&subject=" + "Encrypted:" + btoa(document.getElementById('subject').value) + "&body=" + encodeURIComponent("Copy and decrypt this mail under https://wervice.github.io/projects/WebDesk/messageBoard/decrypt.html : " + btoa(btoa(btoa(btoa(pass + "/" + "Subject: " + document.getElementById('subject').value + "<br>" + document.getElementById('mail').value))))))
    }
}
