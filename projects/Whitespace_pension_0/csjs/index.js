...
function saveFileinBrowser(filename, data, sheme) { if (sheme == "Whitespace") { localStorage.setItem("Whitespace:data:" + filename, data); localStorage.setItem("Whitespace:filesystem:_", localStorage.getItem("Whitespace:filesystem:_") + "<br><button onclick='data = " + data + "; console.log('Opening file.')'>" + "</button>") } else { alert("Invalid sheme!") } }
function openFile() {
    document.documentElement.innerHTML += '<input type=file id=filepicker hidden>'
